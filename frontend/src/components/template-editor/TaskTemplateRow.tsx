...
import { upsertTaskTemplate, deleteTaskTemplate, removeTemplateTaskDependency } from "./api_wrappers";
...
const MODULE_OPTIONS = [
  { value: 'core', label: 'Core' }, { value: 'formula', label: 'Fórmula' },
  { value: 'pack', label: 'Empaque' }, { value: 'brand', label: 'Marca' },
] as const;
...
export default function TaskTemplateRow({ task, allFlatTasks, allRoles, existingStageNames, templateId, scheduleMap, onDelete, onAddChild, depth, onDragStart, onDragEnter, onDrop }: {
  task: any; allFlatTasks: any[]; allRoles: any[]; existingStageNames: string[];
  templateId: string; scheduleMap: Map<number, {startDay: number, endDay: number, slack: number, isCriticalPath: boolean}>; onDelete: (id: number) => void; onAddChild: (parentTask: any) => void; depth: number;
  onDragStart?: (taskId: number, e: React.MouseEvent, coords: {x: number, y: number}) => void;
  onDragEnter?: (taskId: number) => void;
  ...
  const handleSaveEdit = async () => {
    ...
    const result = await upsertTaskTemplate({
      ...
      order: task.order, roleId: parseInt(editData.roleId), isMilestone: editData.isMilestone,
  ...
  const handleAddDep = async (depTask: any) => { 
    ...
    setOptimisticDeps([...optimisticDeps, { id: tempId, dependsOn: depTask }]);
    ...
    if (result.success) {
      ...
      setOptimisticDeps(optimisticDeps.filter(d => d.id !== tempId));
      alert(result.error);
    }
  };

  const handleRemoveDep = async (depRecordId: number, depTaskId: number) => { 
    // Optimistic remove
    setOptimisticDeps(optimisticDeps.filter(d => d.dependsOn.id !== depTaskId));
    
    // We only trigger real remove if it's a real DB record
    if (depRecordId > 0) {
